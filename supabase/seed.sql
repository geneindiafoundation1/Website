-- ---------------------------------------------------------------------------
-- GENE-INDIA Foundation - launch content
-- Run after schema.sql. Safe to re-run: rows are matched on slug and updated.
-- Mirrors src/lib/seed.ts, which the site falls back to before Supabase is
-- connected. Keep the two in step if you edit either.
-- ---------------------------------------------------------------------------
...
delete from public.posts
  where slug in (
    ...
    'reach-without-boundaries'
  );

insert into public.posts (slug, title, excerpt, body, category, cover_url, published, published_at, read_minutes) values
  ('the-foundations-we-forget-to-build', 'The Foundations We Forget to Build', 'We spend enormous energy on houses and weddings. Are we giving the same attention to the young people who will inherit all of this?', 'Kashmir has always stood at the crossroads of different civilizations. Our traditions and cultural influences go back thousands of years, and what we call Kashmiri culture today is really a beautiful mix of all those histories, values, and influences. We inherit much of it from our families, but we also pick up a lot from the society around us, often without even realizing it.
...
Maybe the real question isn''t whether we are doing enough for our children. It is whether our priorities are in the right place. Because one day, our houses will age, the weddings will be forgotten, and the photographs will gather dust. What will remain is the generation we raised, and the values, confidence, compassion, and resilience we gave them. And that, perhaps, is the legacy worth investing in.', 'Reflections', '/blog/foundations.jpg', true, '2026-09-18', 3),
  ('breaking-the-inheritance-of-limitations', 'Breaking the Inheritance of Limitations for a Confident Future', 'Education should never be a luxury. Mentorship is how we stop celebrating only those who escaped the odds, and start changing the odds themselves.', 'Kashmir has witnessed its share of upheavals. We have inherited conflicts we did not create, and we have grown up carrying the weight of experiences that were often beyond our control. And when you grow up in conflict, irrespective of its source, even something as fundamental as education can begin to feel like a luxury.
...
insert into public.team_members (slug, name, role, bio, photo_url, tags, sort_order, published) values
  ('upendra-kaul', 'Dr. Upendra Kaul', 'Director · Interventional Cardiology', 'One of India''s most distinguished interventional cardiologists and a pioneer of the field in the country. He introduced percutaneous transluminal coronary angioplasty (PTCA) to India in the 1980s and established one of the nation''s first balloon angioplasty programs at AIIMS, New Delhi.
...
At GENE-INDIA Foundation, Dr. Kaul brings a lifetime of mentorship experience and a personal commitment to nurturing the next generation of healthcare professionals.', '/team/upendra-kaul.jpg', array['450+ papers', '400+ cardiologists trained', 'Padma Shri', 'Dr. B.C. Roy Award']::text[], 1, true),
  ('tanzila-mukhtar', 'Dr. Tanzila Mukhtar', 'Director · Neurobiology', 'A young neurobiologist whose work sits at the intersection of scientific rigor and social purpose. Driven by a firsthand understanding of how far talent can be limited by a lack of access rather than ability, she co-founded GENE-INDIA Foundation to build the kind of mentorship network she wished existed for students navigating the sciences on their own.

Her research background gives her a deep appreciation for structured guidance and long-term thinking - qualities she now channels into shaping GENE-INDIA''s programs, partnerships, and vision for grassroots impact across India.', '/team/tanzila-mukhtar.jpg', array['Neuroscience', 'Programs & partnerships', 'Grassroots impact']::text[], 2, true)
on conflict (slug) do update set
  name = excluded.name,
  role = excluded.role,
  bio = excluded.bio,
...
-- Retired: remove from any database this seed has already been run against.
delete from public.team_members where slug = 'faisal-kawoosa';
